...
try:
    ...
    - Desarrollo (20 segundos): 3 datos curiosos o útiles sobre IA
    - CTA (últimos 7 segundos): invita a dar like, comentar y compartir
    ...
    # Guion de respaldo
    guion = """
    ¿Sabías que la IA puede crear arte?
    Dato 1: La IA generativa crea imágenes desde texto.
    Dato 2: Los modelos de lenguaje entienden 100+ idiomas.
    Dato 3: La IA ayuda a diagnosticar enfermedades.
    Dale like y comenta qué tema quieres para mañana.
    """
    print("⚠️ Usando guion de respaldo")
...
try:
    # Limitar el texto a 500 caracteres (límite de gTTS)
    ...
    print("⚠️ Usando audio de respaldo (silencio)")
...
try:
    ...
    # Ajustar duración del audio si es más corto
    if audio_clip.duration < 30:
        audio_clip = audio_clip.loop(duration=30)
    elif audio_clip.duration > 30:
        audio_clip = audio_clip.subclip(0, 30)

    # Crear subtítulos (texto en la parte inferior)
    txt_clip = TextClip(
        guion[:350],
        fontsize=45,
        color='white',
...
try:
    ...
    data = {
        ...
        'title': '🤖 Curiosidad de IA del día',
...
try:
    ...
    body = {
        'snippet': {
            ...
            'tags': ['IA', 'inteligencia artificial', 'curiosidades', 'shorts', 'tecnología'],
            'categoryId': '22'  # Categoría: Ciencia y Tecnología
        },
        'status': {
    ...
    print("⚠️ El video está disponible como artefacto descargable")

# ============================================================
# 8. PUBLICAR EN TIKTOK (TIKTOK BUSINESS API, GRATIS CON APROBACIÓN)
# ============================================================
...
if TIKTOK_ACCESS_TOKEN:
    try:
        ...
        print(f"❌ Excepción al publicar en TikTok: {e}")
else:
    print("⚠️ TikTok no configurado (falta TIKTOK_ACCESS_TOKEN), saltando...")
...
# ============================================================
# 10. NOTIFICACIÓN POR TELEGRAM (OPCIONAL)
# ============================================================
try:
    ...
    print(f"⚠️ Error al enviar notificación Telegram: {e}")
